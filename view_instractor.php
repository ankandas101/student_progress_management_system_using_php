...
<?php
if(isset($_GET['id'])){
    $type_ins = array('',"Professor","Assistant Professor","Senior Lecturer","Lecturer");
    $dpt = array("N/A","CSE","EEE","CE");
	$qry = $conn->query("SELECT *,concat(firstname,' ',lastname) as name FROM instructors where id = ".$_GET['id'])->fetch_array();
foreach($qry as $k => $v){
	$$k = $v;
}
}
	...
	<div class="card card-widget widget-user shadow">
      ...
      <div class="card-footer">
        <div class="container-fluid">
        	...
        	<dl>
        		<dt>Designation</dt>
                <dd><?php echo $type_ins[$designation] ?></dd>
        	</dl>
            <dl>
        		<dt>Department</dt>
                <dd><?php echo $dpt[$dept] ?></dd>
        	</dl>
        </div>
    </div>
	</div>
