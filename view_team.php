...
$manager = $conn->query("SELECT *,concat(firstname,' ',lastname) as name FROM instructors where id = $manager_id");
$manager = $manager->num_rows > 0 ? $manager->fetch_array() : array();

$courses = $conn->query("SELECT *,concat(course_code,' | ',course_title) as name FROM course where id = $course_ids");
$courses = $courses->num_rows > 0 ? $courses->fetch_array() : array();

$cls_id = $courses['class_id'];
$cls = $conn->query("SELECT *,concat(semester,' ',year,' - ',sec) as name FROM class where id =$cls_id");
$cls = $cls->num_rows > 0 ? $cls->fetch_array() : array();
			...
			<div class="callout callout-info">
				<div class="col-md-12">
					<div class="row">
						<div class="col-sm-6">
							<dl>
								<dt><b class="border-bottom border-primary">Team Name</b></dt>
								<dd><?php echo ucwords($name) ?></dd>
								<dt><b class="border-bottom border-primary">Class</b></dt>
								<dd><?php echo ucwords($cls['name']) ?></dd>
								<dt><b class="border-bottom border-primary">Course</b></dt>
								<dd><?php echo ucwords($courses['name'])  ?></dd>
								<dt><b class="border-bottom border-primary">Description</b></dt>
								<dd><?php echo html_entity_decode($description) ?></dd>
								<!-- 
								<dt><b class="border-bottom border-primary">Total Mark</b></dt>
								<dd><?php echo html_entity_decode($set_marks) ?></dd>
								-->
						...
						<div class="col-md-6">
							...
							<dl>
								<dt><b class="border-bottom border-primary">Team Instructor</b></dt>
								<dd>
									...
									<?php else: ?>
										<small><i>Instructor Deleted from Database</i></small>
									<?php endif; ?>
								</dd>
							</dl>
						</div>
			...
			<div class="card card-outline card-primary">
				...
				<div class="card-body">
					<ul class="users-list clearfix">
						<?php 
						if(!empty($user_ids)):
							$members = $conn->query("SELECT *,concat(firstname,' ',lastname) as name FROM students where id in ($user_ids) order by concat(firstname,' ',lastname) asc");
						...
						<tbody>
							...
							?>
								<tr>
			                        ...
			                        <td class=""><?php echo ucwords($row['t_marks']) ?></p></td>
...
<script>
	...
	$('#new_mark').click(function(){
		location.href = 'index.php?page=add_mark';
	})
